...
exports.default = async (api) => {
    // generate configs
    const [bundleConfigs, bundlessConfigs] = (0, config_1.normalizeUserConfig)(api.config, api.pkg).reduce((ret, config) => {
    ...
    const sourceFiles = sourceDirs.reduce((ret, dir) => ret.concat(utils_1.glob.sync(`${dir}/**`, {
        cwd: api.cwd,
