...
type FieldPropsTypeBase<Entity, ComponentsType, ExtraProps, FieldPropsType> = ((form: FormInstance<any>, config: ProSchema<Entity, ExtraProps> & {
    type: ComponentsType;
    isEditable?: boolean;
    rowKey?: string;
    rowIndex: number;
...
type UnionSameValueType<Type> = Type extends any ? Type extends ProFieldValueType ? never : Type : never;
export type ValueTypeWithFieldProps<Entity, ComponentsType, ExtraProps, ValueType> = ValueTypeWithFieldPropsBase<Entity, ComponentsType, ExtraProps, ProFieldValueType | UnionSameValueType<ValueType> | undefined>;
...
/** @deprecated */
export type ProSchemaValueType<ValueType> = (ValueType | ProFieldValueType) | ProFieldValueObjectType;
export type ProSchemaFieldProps<T> = Record<string, any> | T | Partial<InputProps>;
/** 各个组件公共支持的 render */
export type ProSchema<Entity = Record<string, any>, ExtraProps = unknown, ComponentsType = ProSchemaComponentTypes, ValueType = 'text', ExtraFormItemProps = unknown> = {
